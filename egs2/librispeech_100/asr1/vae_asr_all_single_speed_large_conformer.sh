...
adv_liststr="recon 5 asr 30 adv 30 asradv 30 reinit_adv 20"
...
project_name="vae_xworkingvae_single_speed_larger_conformer_june_17"
    ...
    if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ]; then
        ...
        ${python} -m espnet2.bin.launch \
            ...
            ${python} -m espnet2.bin.asr_train \
                ...
                --asr_weight_factor "${asr_weight_factor}" \
                --non_linguistic_symbols "${nlsyms_txt}" \
                --cleaner "${cleaner}" \
                --g2p "${g2p}" \
                --allow_variable_data_keys true \
